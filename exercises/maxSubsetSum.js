...
function mergeSort(array) {
    ...
    while (i1 < part1.length || i2 < part2.length) {
        if (i2 >= part2.length || (i1 < part1.length && part1[i1].value >= part2[i2].value))
            result.push(part1[i1++]);
        else
            result.push(part2[i2++]);
    }
    return result;
}
...
function maxSubsetSum(arr) {
    let sum = 0;
    let temp = arr.map((value, index) => {return {index, value}});
